...
function Beam({ index }: { index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="h-full w-full bg-gradient-to-b from-secondary via-secondary/30 via-75% to-secondary/50" />
      </div>
    </div>
  );
}
...
function Background() {
  ...
  return (
    <div
      ref={containerRef}
      className="-z-1 absolute inset-0 flex h-full w-full flex-row justify-between bg-gradient-to-t from-primary to-primary/40 "
    >
      <div
        style={{
          background:
            "radial-gradient(50% 50% at 50% 50%,#3b2f35 0%,rgb(7,42,57) 50%,rgba(7,42,57,0) 100%)",
        }}
...
export default function AnimatedBeam({
  ...
  return (
    ...
    >
      <Background />
      <div className="relative h-full w-full max-w-7xl">{children}</div>
    </div>
  );
}
